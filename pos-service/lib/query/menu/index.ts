...
export async function findAllMenuWithCompanyNumber(company_number: string) {
  if (!company_number) throw Error('wrong parameter')

  const responses = await query([
    {
      sql: 'SELECT * FROM menu WHERE company_number=?',
      values: [company_number],
    },
  ])

  const found = responses[0].rows as RowDataPacket[]
