  ...
  try {
    ...
    if (user) {
      ...
      if (user.type === 'company') {
        const companyProfile = await search.findCompanyByEmail(req.params.email)
...
router.get('/social/fcm', async (req, res) => {
  try {
    const social = await search.findSocialFCMTokenByEmail(req.query.email)
    if (social) {
      res.json(social)
    } else {
      res.status(404)
...
/** fcm 토큰 조회 */
router.get('/social/:social_id/fcm', async (req, res) => {
  try {
